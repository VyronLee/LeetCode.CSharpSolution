// ------------------------------------------------------------
//         File: Solution0724.cs
//        Brief: Solution0724.cs
//
//       Author: VyronLee, lwz_jz@hotmail.com
//
//      Created: 2024-2-19 18:0
...
namespace LeetCode
{
    public class Solution0724
    {
        public int PivotIndex(int[] nums) {
            ...
            postfixSums[nums.Length - 1] = nums[^1];
            for (var i = nums.Length - 2; i >= 0; i--) {
                postfixSums[i] = postfixSums[i + 1] + nums[i];
            }
            // 寻找中心下标
            for (var i = 0; i < nums.Length; i++) {
                if (prefixSums[i] == postfixSums[i]) {
                    return i;
                }
            }
            return -1;
